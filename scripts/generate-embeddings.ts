...
import { getAllNotionPagesMarkdown } from './get-all-notion-pages-md';
...
class MarkdownEmbeddingSource extends BaseEmbeddingSource {
  ...
  constructor(source: string, notionPageId: string, markdownString: string) {
    super(source, notionPageId, markdownString);
  }

  async load() {
    const { checksum, meta, sections } = processMdxForSearch(
      this.markdownString,
    );
...
async function generateEmbeddings() {
  ...
  const embeddingSources: EmbeddingSource[] = (
    await getAllNotionPagesMarkdown()
  ).map(
    ({ id, contents }) => new MarkdownEmbeddingSource('notion', id, contents),
  );
  ...
  for (const embeddingSource of embeddingSources) {
    const { type, source, notionPageId } = embeddingSource;
    ...
    try {
      ...
      const { error: fetchPageError, data: existingPage } = await supabaseClient
        .from('nods_page')
        .select('id, notion_page_id, checksum')
        .filter('notion_page_id', 'eq', notionPageId)
        .limit(1)
        .maybeSingle();
      ...
      if (existingPage) {
        if (shouldRefresh) {
          console.log(
            `[${notionPageId}] Refresh flag set, removing old page sections and their embeddings`,
          );
        } else {
          console.log(
            `[${notionPageId}] Docs have changed, removing old page sections and their embeddings`,
          );
        }
      ...
      console.log(
        `[${notionPageId}] Adding ${sections.length} page sections (with embeddings)`,
      );
      for (const { slug, heading, content } of sections) {
        ...
        try {
          ...
          console.error(
            `Failed to generate embeddings for '${notionPageId}' page section starting with '${input.slice(
              0,
              40,
            )}...'`,
          );
      ...
      console.error(
        `Page '${notionPageId}' or one/multiple of its page sections failed to store properly. Page has been marked with null checksum to indicate that it needs to be re-generated.`,
      );
      console.error(err);
    }
  }
